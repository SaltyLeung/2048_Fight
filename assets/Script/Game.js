...
var Game = cc.Class({
    ...
    randomCreate() {
        ...
        for(var i = 0; i < 16; ++i) {if(this.moveableList[i]!=null)console.log("moveableNum"+i+": "+this.moveableList[i].getComponent(require("Square")).number);}
    ...
    moveDown() {
        ...
        for(var i = 0; i < 4; ++i) {  //列
            for(var j = i; j < 16; j += 4) {   //每列中每个
                ...
                for(var k = j - 4; k >= 0; k -= 4) {    //计算目标
                    if(this.positionList[k].number == 0) { targetIndex = k; hasHole[Math.floor(j/4)] = true; continue; }
        ...
        for(var i = 0; i < 16; ++i) {
            if((this.moveableList[i] != null) && (this.moveableList[i].getComponent(require("Square")).toDestroyed == true))   { console.log("Destroy" +i);var temp = this.moveableList[i]; setTimeout(()=>{temp.destroy();},0.5);   this.moveableList[i] = null; this.positionList[i].number = 0;}
        }    //删除多余卡片 
